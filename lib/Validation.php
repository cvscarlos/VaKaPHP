 ...
 *
 * @contribute	Carlos Vinicius (VakaPHP)
 ********************************** 80 Columns *********************************
 */
class Validation
{
	...
	public $error_prefix = '<div class="form_error error">';
	...
	public function run(array $fields)
	{
		if($_POST)
		{
			...
			{
				$r=explode('|',$r);
				if(!in_array('required',$r)&&!Functions::post($f))
					continue;
				$this->_rules($f,Functions::post($f),$r);
				$this->validate_token();
			}
			if(!$this->errors)
				return 1;
	...
	{
		$this->errors[$field]=vsprintf(Functions::messages('validation_'.$name),array_merge(array($field),$params));
	}
	...
	{
		if($data=trim(Functions::str($data)))return$data;$this->_set_error($field,'required');return FALSE;
	}
	...
	{
		if (isset($_POST[$field2])&&$data===Functions::post($field2))return TRUE;$this->_set_error($field,'matches',array($field2));return FALSE;
	}
